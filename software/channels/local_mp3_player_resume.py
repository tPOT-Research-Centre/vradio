...
class LocalMP3Channel(BaseChannel):
    ...
    async def playNewAudio(self):
        ...
        print(f"[{self.name}] Starting playback of: {self.file_path} from offset {self.offset}")
        self.process = await asyncio.create_subprocess_exec("ffplay", "-nodisp", "-autoexit", "-ss", str(self.offset), self.file_path,stdin=subprocess.PIPE, stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
